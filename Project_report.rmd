...
title(sub = "Figure 1: The kernel plot clearly shows the distribution of the response variable is not normal.",
      cex.sub = 1.2, line = 4)
```
...
As shown on Figure 2, after using log, square root, inverse transformation, and inverse log transformation, we found that all of them can not handle the skewness and multi-peaks of the response variable. So we decided that nonparametric regression is the best choice for this dataset.
